...
void M3DLoader::ReadSkinnedVertices(std::ifstream& fin, UINT numVertices, std::vector<Vertex>& vertices)
{
	std::string ignore;
    vertices.resize(numVertices);
...
void M3DLoader::ReadBoneKeyframes(std::ifstream& fin, UINT numBones, BoneAnimation& boneAnimation)
{
    ...
    for(UINT i = 0; i < numKeyframes; ++i)
    {
	    ...
	    boneAnimation.Keyframes[i].TimePosition  = t;
        boneAnimation.Keyframes[i].Position      = p;
	    boneAnimation.Keyframes[i].Scale         = s;
	    boneAnimation.Keyframes[i].Quaternion    = q;
    }
