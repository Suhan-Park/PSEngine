...
public:
	...
	bool LoadM3d(const std::string& filename, 
		...
		std::vector<Subset>& subsets,
		std::vector<M3dMaterial>& mats,
		class SkinnedMeshRenderer& skinnedMeshRenderer);
...
private:
	...
	void ReadSkinnedVertices(std::ifstream& fin, UINT numVertices, std::vector<Vertex>& vertices);
	...
	void ReadAnimationClips(std::ifstream& fin, UINT numBones, UINT numAnimationClips, std::unordered_map<std::string, class AnimationClip>& animations);
	void ReadBoneKeyframes(std::ifstream& fin, UINT numBones, class BoneAnimation& boneAnimation);
};
